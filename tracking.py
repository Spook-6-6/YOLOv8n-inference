...
class Tracker:
    def __init__(self, model_path='yolov8n.pt', tracker_type='csrt'):
    ...
    def _process_multi_tracker(self, frame):
        ...
        return draw_boxes(frame, normal, selected, self.model)
...
class NanoTracker:
    ...
    def init(self, frame, bbox):
        try:
            ...
            backbone_path = os.path.join(current_dir, "nanotrack_backbone_sim.onnx")
            neckhead_path = os.path.join(current_dir, "nanotrack_head_sim.onnx")
...
class ViTTracker:
    ...
    def init(self, frame, bbox):
        try:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            vit_path = os.path.join(current_dir, "object_tracking_vittrack_2023sep.onnx")
